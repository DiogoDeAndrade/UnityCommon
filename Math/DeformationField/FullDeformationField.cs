...
public class FullDeformationField
{
    ...
    public Vector3 DeformPositionFromNodeFrames(Vector3 position, List<Frame> currentNodeFrames)
    {
    ...
    public Vector3 DeformPositionFromNodeFrames(Vector3 position, Func<int, Frame> getCurrentNodeFrame)
    {
    ...
    public Frame DeformFrame(Frame frame, List<Frame> currentNodeFrames)
    {
        ...
        return DeformFrameInternal(frame, (nodeIndex) => (nodeIndex < currentNodeFrames.Count) ? currentNodeFrames[nodeIndex] : (Frame?)null);
    }

    public Frame DeformFrame(Frame frame, Func<int, Frame> getCurrentNodeFrame)
    {
        if (getCurrentNodeFrame == null) return frame;

        return DeformFrameInternal(frame, (nodeIndex) => getCurrentNodeFrame(nodeIndex));
    }

    Frame DeformFrameInternal(Frame frame, Func<int, Frame?> getCurrentNodeFrame)
    {
        DeformationFieldWeights fieldWeights = GetWeights(frame.position);

        if ((fieldWeights.weights == null) || (fieldWeights.nodeId == null)) return frame;
        ...
        for (int i = 0; i < fieldWeights.weights.Length; i++)
        {
            ...
            Frame? currentFrame = getCurrentNodeFrame(nodeIndex);
            if (!currentFrame.HasValue) continue;

            AccumulateMatrix(ref blended, NodeDeformMatrix(nodeIndex, currentFrame.Value), weight);
            weightSum += weight;
        }

        if (weightSum <= DistanceEpsilon) return frame;
        ...
        return new Frame(blended.MultiplyPoint3x4(frame.position),
