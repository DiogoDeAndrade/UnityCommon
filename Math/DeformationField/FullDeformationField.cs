...
public partial class FullDeformationField
{
    ...
    /// mode rather than as a descriptor because no blender has parameters yet; the moment one does,
    /// this should become a descriptor generated by the blender, exactly as weightDescriptor is.
    /// </summary>
    [SerializeField, HideInInspector]
    ...
    /// How CreateBlender combines the node transforms. Carried rather than used during the build -
    /// the blend happens per query - but it changes every position this field produces, so it is
    /// recorded alongside the settings that do shape the build.
    /// </param>
