...
public partial class FullDeformationField
{
    public abstract class WeightResolver
    {
        ...
        ///
        /// **Distances arrive sorted nearest-first, and that is a precondition rather than an
        /// observation.** ComputeWeights gathers them out of a cell that SortInfluences has already
        /// ordered by (distance, node). Entmax's threshold search relies on it; a resolver handed an
        /// unsorted array would return a wrong answer rather than an error.
        ///
        /// Every distance passed in is real - the gather drops empty slots and MaxValue - so a
        /// resolver never has to test for those. It writes into a buffer the caller owns, because
        /// this runs once per cell and returning a fresh array would allocate once per cell too.
        ///
        /// Weights are expected to sum to one. Writing all zeros is allowed and means "nothing acts
        /// here", which the field already treats as a result rather than a gap.
        /// </summary>
        public abstract void ComputeCellWeights(float[] distances, int count, float[] weights);

        /// <summary>
        /// The mapping and its parameters, short and stable, for the golden dump and for the check
        /// that refuses a comparison across a weighting change.
        ///
        /// Generated from the parameters rather than written beside them, so it cannot drift out of
        /// sync with what the resolver actually does - which is the one way a recorded setting is
        ...
        ///
        /// Here rather than in ComputeWeights because only some mappings want it, and a caller
        /// applying it on their behalf would be deciding something that belongs to the mapping - the
        /// legacy inverse-distance path in particular must not receive it, or it stops reproducing.
        ///
        /// Dividing by the *furthest kept* distance pins the last influence to a constant score,
        /// whatever the mapping: with sigma fixed, the k-th node always weighs the same amount before
        /// normalization. That is a statement about what the k-th influence means and not an
        /// accident - the OriginalED binding weight does the same thing, and goes to exactly zero
        /// there. The alternative is a world-space parameter, which is brittle against voxel density
        /// and piece scale in exactly the way a fixed softmax temperature is.
        /// </summary>
        protected static void NormalizeToFurthest(float[] distances, int count, float[] normalized)
        {
            ...
            // Every distance identical, or every one zero. Both leave the mapping nothing to
            // discriminate on, and scaling by an arbitrary number would invent a spread that is not
        ...
        /// <summary>
        /// Divides through by the sum, or writes zeros when there is no sum to divide by. The single
        /// point every score-based mapping normalizes at, so none of them can quietly skip it.
        /// </summary>
    ...
    /// <summary>
    /// Normalized inverse distance, 1/max(d, eps) over the sum, splitting evenly between any nodes at
    /// distance zero.
    ///
    /// **This is the legacy mapping and it must stay bit-identical.** It is what every structure
    /// golden was captured against, so the arithmetic below is the original arithmetic in the
    /// original order - including that the even-split branch fires on the whole set the moment one
    /// distance is at or under epsilon, and that max(d, eps) is applied inside both the sum and the
    /// division rather than once.
    ///
    /// The even-split branch exists only because 1/d diverges. It is not inherited by the mappings
    /// below, none of which has a singularity at zero to guard - and since seeds gained their true
    ...
    ///
    /// Sigma is a fraction of the furthest kept distance when normalization is on, and a world-space
    /// length when it is off. On is the intended setting: a world-space sigma is brittle against both
    /// the voxel density and the scale of the map piece, in exactly the way a fixed softmax
    /// temperature is. Off exists so that brittleness can be demonstrated rather than asserted.
    ///
    /// p = 2 is the true Gaussian. Higher p flattens the centre and steepens the shoulder, towards a
    /// compact-support kernel without needing a support radius.
    /// </summary>
    ...
    ///
    /// The reason to want it: it can drive weights to exact zero through the normalization itself
    /// rather than through a cutoff, so the effective number of influences varies from place to place
    /// on its own. Nothing else here can do that - every mapping above gives every node in the set a
    /// nonzero share however far away it is.
    ///
    /// weights_i = [(alpha - 1) * z_i - tau]_+ ^ (1 / (alpha - 1)), with z_i = -d_i / temperature and
    /// tau chosen so the weights sum to one. Alpha 1 would be softmax (dense, never zero) and alpha 2
    /// sparsemax; 1.5 sits between and is the usual choice.
    ///
    /// **Tau is found by bisection rather than in closed form.** The exponent is only a convenient 2
    /// at alpha = 1.5, and a solver that works for one alpha is a solver that silently misbehaves at
    /// the others. The sum is monotonically decreasing in tau, the set holds at most a handful of
    /// nodes, and this runs once per cell - so the general method costs nothing worth saving.
    /// </summary>
    public sealed class EntmaxWeights : WeightResolver
    {
        ...
        public override void ComputeCellWeights(float[] distances, int count, float[] weights)
        {
            ...
            // Near is high, so the scores are negated distances. Sorted nearest-first on the way in
            // means score[0] is the largest.
            float highest = -source[0] / temperature;
            float lowest = -source[count - 1] / temperature;

            // At the upper bound every term is clipped to zero, so the sum is 0 and below 1. At the
            // lower bound the smallest score alone contributes at least 1, so the sum is at least 1.
            ...
            // The bisection lands close to a sum of one but not on it, and a blend that does not sum
            // to one is a blend that shrinks or inflates the deformation. Renormalizing costs a pass
            // and removes the tolerance from the answer, at the price of nothing: it is a positive
            // rescale, so it cannot resurrect a weight the threshold set to zero.
            NormalizeInPlace(weights, count);
        }
    }
}
